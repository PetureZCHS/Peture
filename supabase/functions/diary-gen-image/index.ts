...
2. 未知实体的语义泛化与人脸规避（Semantic Generalization & Face Avoidance）：
   - 泛化地点：将具体的专有名词翻译为通用的优美场景。
   - 弱化次要角色：将次要宠物或人类翻译为泛化词汇，并强制将其推向背景。如果人类不是核心互动对象，请勿在提示词中体现。
   - 绝对避免清晰人脸：如果根据日记内容互动必须出现人类（如喂食、抚摸），请通过局部构图或背影来隐藏面部，强制在提示词中加入如「only showing hands」、「faceless」、「view from behind」、「face out of frame」等限制词，确保不出现任何清晰可辨的人类面孔。
...
serve(async (req)=>{
  ...
  try {
    ...
    if (!pet_id || !diary_content) {
      return jsonResponse({
        error: "Missing pet_id or diary_content."
      }, 400, origin);
    }
    // 2. 校验权限与用户身份
    ...
    // 3. 频控守卫校验
    const { data: guard, error: guardError } = await supabase.rpc("img_gen_check_guard", {
      p_user_id: userId
    }).single();
    if (guardError || !guard) {
      return jsonResponse({
        error: "Rate-limit guard failed",
    ...
    const { data: petData, error: petError } = await supabase.rpc("get_pet_for_diary", {
      p_pet_id: pet_id,
      p_user_id: userId
    }).single();
    if (petError) {
    ...
    if (petError || !petData) {
      return jsonResponse({
        error: "Pet not found or permission denied."
      }, 404, origin);
    }
    if (!petData.life_photo) {
      return jsonResponse({
        error: "Pet does not have a life_photo set."
      }, 400, origin);
    }
    ...
    const { data: signedData, error: signedError } = await supabase.storage.from(sourceBucket).createSignedUrl(sourcePath, 60 * 5);
    if (signedError || !signedData?.signedUrl) {
      return jsonResponse({
        error: "Failed to create signed url for life_photo",
...
请根据以上输入，直接输出纯正的英文提示词（Prompt）：`;
    ...
    if (!generatedPrompt) {
      ...
      return jsonResponse({
        error: "Failed to generate prompt from diary.",
        detail: "Check Supabase logs for SiliconFlow response details."
      }, 500, origin);
    }
    // 修复换行符正则，使用真正的换行符匹配 \\n 而非字符串的 \\\\n
    ...
    if (!generatedPrompt) {
      ...
      return jsonResponse({
        error: "Failed to generate prompt from diary (cleaned up as empty)."
      }, 500, origin);
    }
    ...
    try {
      dashscopeResp = await fetch(DASHSCOPE_API_URL, {
        ...
        body: JSON.stringify({
          ...
          parameters: {
            n: 1,
            negative_prompt: "human face, clear face, detailed facial features, front view of person",
            prompt_extend: true,
            watermark: false,
            size: imageSize
          }
    ...
    if (!dashscopeResp.ok) {
      return jsonResponse({
        ...
        detail: await dashscopeResp.text()
      }, 500, origin);
    }
    ...
    if (!generatedImageUrl) {
      return jsonResponse({
        error: "Image URL not found in DashScope response"
      }, 500, origin);
    }
    ...
    try {
      ...
      if (err instanceof Error && err.name === "AbortError") {
        return jsonResponse({
          error: "Generated image download timeout"
        }, 504, origin);
      }
      throw err;
    }
    ...
    const { error: uploadError } = await supabase.storage.from("ai-wallpapers").upload(generatedStoragePath, arrayBuffer, {
