...
const apiKey = Deno.env.get("DIFY_DIARY_V4_API_KEY") ?? Deno.env.get("DIFY_DIARY_API_KEY");
...
function shouldTriggerOutputAudit(pendingSegment, lastAuditAt) {
...
Deno.serve(async (req)=>{
  ...
  if (!inputs || typeof inputs !== "object") {
    return new Response(JSON.stringify({
      error: "Missing required 'inputs' object in request body."
    }), {
      status: 400,
      headers: {
        ...corsHeaders,
      ...
      async transform (chunk, controller) {
        ...
        while(sseBuffer.includes("\n\n")){
          ...
          try {
            ...
            if (eventName === "text_chunk") {
              ...
              if (piece) {
                ...
                if (shouldTriggerOutputAudit(pendingSegment, lastAuditAt)) {
                  ...
                  const contextPrefix = prefixLength > 0 ? visibleOutputBuffer.slice(0, prefixLength).slice(-OUTPUT_AUDIT_TAIL_CONTEXT) : "";
                  ...
                  if (!outputModeration.passed) {
                    ...
                    const safePayload = `data: ${JSON.stringify({
                      event: "text_chunk",
                      data: {
                        text: `该内容未通过审核（traceId: ${traceId}）`
                      }
                    })}\n\n` + `data: ${JSON.stringify({
            ...
            const outText = eventData?.data?.text?.toString?.() ?? "";
  ...
  const outputText = respJson?.data?.outputs?.text?.toString?.() ?? "";
  if (outputText) {
