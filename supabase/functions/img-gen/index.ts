...
Deno.serve(async (req)=>{
  ...
  try {
    const { file_name, style } = await req.json();
    if (!file_name || !style) {
      return jsonResponse({
        error: "file_name and style are required"
      }, 400);
    }
    ...
    const { data: presetData, error: presetError } = await supabase.from("ai_image_presets").select("prompt, aspect_ratio").eq("id", style).single();
    ...
    for(let attempt = 1; attempt <= maxAttempts; attempt++){
      ...
      const { data: files, error: listError } = await supabase.storage.from("ai-wallpapers").list(originalFolder, {
        limit: 10,
        search: file_name
      });
      if (listError) {
        return jsonResponse({
          error: "Failed to check uploaded file",
    ...
    const { data: signedData, error: signedError } = await supabase.storage.from("ai-wallpapers").createSignedUrl(targetFilePath, 60 * 5);
    ...
    if (!generatedChoices || generatedChoices.length === 0) {
      return jsonResponse({
        error: "No image output from upstream",
        detail: data
      }, 500);
    }
    const generatedImageUrl = generatedChoices[0]?.message?.content?.[0]?.image;
    if (!generatedImageUrl) {
      return jsonResponse({
        error: "Image URL not found in upstream response",
        detail: data
      }, 500);
    }
    let imgResp;
