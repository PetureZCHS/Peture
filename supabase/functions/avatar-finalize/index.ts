...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: fileBytes, error: downloadError } = await serviceClient.storage.from(AVATARS_TEMP_BUCKET).download(tempPath);
    if (downloadError || !fileBytes) {
      throw new Error(downloadError?.message ?? "头像下载失败");
    }
    const finalPath = `${userId}/avatar.jpg`;
    const { error: uploadError } = await serviceClient.storage.from(AVATARS_BUCKET).upload(finalPath, fileBytes, {
    ...
    await serviceClient.storage.from(AVATARS_TEMP_BUCKET).remove([
      tempPath
    ]);
