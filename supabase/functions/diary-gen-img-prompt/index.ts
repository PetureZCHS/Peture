...
serve(async (req)=>{
  ...
  try {
    ...
    if (!pet_id || !diary_content) {
      return new Response(JSON.stringify({
        error: "Missing pet_id or diary_content in request body."
      }), {
        status: 400,
        headers: {
          "Content-Type": "application/json",
    ...
    const { data: petData, error: petError } = await supabase.from("pets").select("type, breed, name, gender, age").eq("id", pet_id).eq("user_id", userId).single();
    if (petError || !petData) {
      return new Response(JSON.stringify({
        error: "Pet not found or you don't have permission to access it."
      }), {
        status: 404,
        headers: {
          "Content-Type": "application/json",
