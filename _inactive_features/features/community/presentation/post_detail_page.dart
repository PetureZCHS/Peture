...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  Future<void> _checkFollowStatus() async {
    if (_authorId == null) {
      debugPrint('_checkFollowStatus: authorId 为空');
      return;
    }
    debugPrint('检查关注状态: authorId=$_authorId');
    final following = await _supabase.isFollowing(_authorId!);
    debugPrint('关注状态结果: $following');
    if (!mounted) return;
    setState(() {
      _isFollowing = following;
      debugPrint('设置关注状态: $_isFollowing');
    });
  }
                  ...
                  builder: (context, snapshot) {
                    ...
                    return TextButton(
                      onPressed: () async {
                        ...
                        debugPrint('关注操作结果: $result');
                        if (!mounted) return;
                        setState(() {
                          _isFollowing = result;
                          _authorId = authorId; // 确保 _authorId 已设置
                          debugPrint('更新后状态: $_isFollowing');
                        });
