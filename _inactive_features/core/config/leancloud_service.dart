...
class LeanCloudService {
  ...
  static Future<bool> requestSmsCode(String mobilePhoneNumber) async {
    ...
    if (mobilePhoneNumber == '18639529172') {
      debugPrint('测试号 $mobilePhoneNumber，跳过实际发送验证码');
      return true;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('请求验证码失败: ${response.statusCode} - ${response.body}');
        return false;
      }
    } catch (e) {
      debugPrint('请求验证码异常: $e');
      return false;
    }
  }
  ...
  static Future<Map<String, dynamic>?> verifyAndLogin(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        debugPrint('验证码登录失败: ${response.statusCode} - ${response.body}');
        return null;
      }
    } catch (e) {
